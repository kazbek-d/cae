...
pub async fn get_watchlist(pool: &PgPool) -> eyre::Result<Vec<Address>> {
    let rows = sqlx::query!("SELECT address FROM watch_list")
        .fetch_all(pool)
        .await?;
    Ok(rows
        .into_iter()
        .map(|r| Address::from_slice(&r.address))
        .collect())
}
...
pub async fn save_native_transfer(
    pool: &PgPool,
    chain_id: u64,
    tx_hash: B256,
    amount: U256,
...
pub async fn get_unprocessed_logs_by_chain(
    pool: &PgPool,
    chain_id: u64,
) -> eyre::Result<Vec<sqlx::postgres::PgRow>> {
...
pub async fn get_or_discover_token<P: Provider>(
    pool: &PgPool,
    provider: Arc<P>,
    chain_id: u64,
    addr: Address,
) -> eyre::Result<(String, i32)> {
    if addr == Address::ZERO {
        return Ok(("ETH".into(), 18));
    }
    if let Some(row) = sqlx::query!(
        "SELECT symbol, decimals FROM token_metadata WHERE address = $1",
        addr.as_slice()
    )
    .fetch_optional(pool)
    .await?
...
pub async fn mark_processed(pool: &PgPool, id: i32) -> eyre::Result<()> {
    sqlx::query!(
        "UPDATE transaction_logs SET is_processed = TRUE WHERE id = $1",
        id
    )
    .execute(pool)
    .await?;
